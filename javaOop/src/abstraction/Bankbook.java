...
public class Bankbook implements Account {
	...
	public void deposit(int money) {
		...
		// 파라미터로 던질 경우 에러가 발생하는 것을 막기 위한 방어코딩
		if(money <= 0){
	...
	/*
	 Java 에서는 자주 사용되는 클래스와 인터페이스를 이미 생성 또는 구현한 뒤
	 개발자들이 이를 가져다 쓰게끔 하고 있습니다.
	 예를들면, 스캐너, 시스템 같은 것입니다.
	 이처럼 미리 만들어진 클래스와 인터페이스를 API라고 합니다.
	 그 중 최상위 객체를 Object라고 하며, 이 Object의 클래스는 자바에서 객체로 선언된 것이라면
	 무조건 상속을 받습니다. 그래서 Object의 메소드들은 자바객체라면 어느것이든 사용할 수 있습니다.
